    ...
    <div class=" mx-auto col-md-5">
      <div class="card shadow-lg bg-white">
       <div class="card-header bg-info">
            <h2 class="card-title text-center font-weight-bolder text-uppercase text-white-50" >Register Form</h2>
        </div>
       
        <div class="card-body">
	    <form action="<?php echo base_url();?>Users/save_user" method="post" enctype="multipart/form-data" >
            ...
            <label class="font-weight-bold" for="country" value="">Country</label>
                    <select class="form-control" name="country_name" id="country_id" class="txt"  >
                        <option  value="<?php echo @$res_data['country_name']; ?>">--- Select Countries--- </option>
                        <?php
                       
                        foreach($country_data as $country){?>
                            <option value = <?php echo  $country['id']; ?>  <?php if($country['country_name'] == $country['id']) { ?> selected="selected" <?php } ?>   > <?php echo $country['country_name']; ?> </option>
                          <?php }?>  
                    </select>
            </div>
            <div class="form-group " >
            ...
            <label class="font-weight-bold"  for="file" class="custom-uploader">Upload Your File</label> 

            <input id="file" type="file" name="file" 
             accept="image/jpeg,image/gif,image/png,application/pdf,image/x-eps" value="file">


            </div>
            <input type="hidden" name="edit_id" value="<?php echo @$res_data['id']?>">
            <center class="mt-3">
            <input type="submit" name= "submit" class="btn btn-info w-50" value="submit">
            </center>
          </form>
